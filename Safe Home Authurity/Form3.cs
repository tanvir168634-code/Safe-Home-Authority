...
namespace Safe_Home_Authurity
{
    public partial class ownerlogin : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string connectionString = "Data Source=DELL-3410\\SQLEXPRESS;Initial Catalog=abbbb;Integrated Security=True;Trust Server Certificate=True";
            // Step 4: Define the SQL query
            // Using COLLATE to make the Name case-sensitive
            string query = "SELECT COUNT(*) FROM Table_1 WHERE ID = @Id AND PASS COLLATE SQL_Latin1_General_CP1_CS_AS = @Name";*/
            ...
            // Step 4: Define the SQL query
            // Using COLLATE to make the Name case-sensitive
            string query = "SELECT COUNT(*) FROM owner WHERE o_id = @Id AND o_pass COLLATE SQL_Latin1_General_CP1_CS_AS = @Name";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        //f3.Show();
                        ownerdashboard form4 = new ownerdashboard(userId);
                        form4.Show();
                        this.Hide();
                    }
                    else
